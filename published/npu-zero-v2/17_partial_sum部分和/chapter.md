...
本章解决一个很具体的问题：一个输出值往往不是一次乘法得到的，而是一长串乘法结果逐步加出来的。这个“还没有完全加完、但已经有一部分贡献”的中间值，就是 partial sum，中文可以先叫“部分和”。如果读者能看见部分和从 0 开始、被一项项乘积推高、最后才变成输出，就能理解为什么 NPU 需要暂存、累加、回读、合并这些看似麻烦的动作。

| 核心问题 | 本章给出的回答 |
| --- | --- |
| partial sum 是什么？ | 某个输出值在累加过程中的中间和，不是最终输出，也不是单个乘积。 |
| 为什么要暂存？ | 因为一个输出可能跨通道、跨卷积核位置、跨 tile 分多次完成，中间和不能丢。 |
| 为什么会影响硬件？ | 部分和放在 PE、本地寄存器、片上 buffer 还是外部内存，会直接影响带宽、能耗和阵列利用率。 |
| 和 dataflow 有什么关系？ | 许多 dataflow 的核心差异，就是让输入、权重或部分和中的哪一种尽量停在本地。 |

学习路径可以按四步走：先看一个小点积里的 `sum` 如何变化；再把这个 `sum` 命名为部分和；再看多通道卷积为什么会产生多个阶段的部分和；最后把它连接到 NPU 的累加器、buffer 和 dataflow。

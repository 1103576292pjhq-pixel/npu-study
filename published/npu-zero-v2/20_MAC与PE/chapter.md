...
第 19 章把循环、tile、dataflow 映射到了硬件资源；本章把镜头拉近，专门看阵列里的一个小格子。前面我们已经知道 `sum = sum + a*b` 是许多神经网络计算的底层动作，也知道 partial sum 不能随便丢。本章要回答的是：这个动作在硬件里由什么东西完成？一个 PE 到底比一个 MAC 多了什么？为什么 NPU 文档总是说“PE 阵列”，而不是只说“很多乘法器”？

## 本章知识全景图

| 核心问题 | 本章给出的回答 |
| --- | --- |
| MAC 是什么？ | MAC 是 multiply-accumulate，形式是 `acc_next = acc + a*b`，可以指一次动作，也可以指执行这个动作的硬件单元。 |
| PE 是什么？ | PE 是 Processing Element，处理元素，是阵列中可被调度的小计算格子，通常包含一个或多个 MAC、局部寄存器、累加器、控制和相邻通信端口。 |
| MAC 和 PE 有何区别？ | MAC 更像“乘加 datapath”；PE 更像“能接收数据、保存状态、按节拍工作的本地计算站”。 |
| 为什么要关心 PE？ | 循环映射、部分和保存、数据复用、阵列利用率，最终都要落到 PE 的输入、输出、寄存器和控制上。 |

学习路径很直接：先把 MAC 当成一个动作看；再把动作包进一个能存状态的 PE；接着用手算例题看 PE 的累加器如何变化；最后把 PE 连接回 NPU 阵列、buffer 和 dataflow。
...
- 正式定义 MAC，并区分“MAC 动作”和“MAC 单元”。
- 正式定义 PE，并说清 PE 至少需要哪些组成部分。
...
- PE 的“正式定义”要允许不同 NPU 微架构差异，但共同点是：它是阵列映射时的本地计算与暂存单位。
...
8. 在完整例题中，为什么 PE0 和 PE1 可以共享输入向量，但不能共享同一个累加器？

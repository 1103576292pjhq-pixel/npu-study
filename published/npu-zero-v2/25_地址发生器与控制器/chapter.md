...
前面章节已经把数据放进片上 buffer，也让 DMA 和双缓冲把数据搬起来。但硬件还需要回答两个非常具体的问题：下一拍读哪个地址？什么时候启动 DMA、什么时候启动 PE、什么时候切换 buffer、什么时候写回？地址发生器负责把循环计数变成真实地址，控制器负责把整个执行过程组织成有顺序、有握手、有停止条件的状态机。

| 核心问题 | 本章给出的回答 |
| --- | --- |
| 地址发生器是什么？ | 根据 base、stride、循环计数和 layout，自动生成 input/weight/output/psum 的读写地址。 |
| 控制器是什么？ | 管理 DMA、buffer、PE 阵列和状态切换的控制逻辑。 |
| 为什么不能手写固定地址？ | 不同 tile、通道、layout、stride、padding 都会改变地址序列。 |
| 本章边界是什么？ | 讲清地址和状态的最小硬件直觉，不展开复杂 SoC 总线协议。 |

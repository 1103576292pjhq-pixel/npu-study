...
本章解决一个 NPU 设计里的核心问题：为什么很多 NPU 喜欢用 int8、int16、int32 这些整数格式来跑神经网络，而不是一直使用浮点数。量化的基本思想是把连续的真实数近似映射到有限个整数格点上；定点数的基本思想是用整数和固定小数点位置来表示小数。它们共同决定了 NPU 的乘法器大小、累加器位宽、带宽、功耗和最终精度。

| 核心问题 | 本章给出的回答 |
| --- | --- |
| 量化是什么？ | 用 scale 和 zero point 把真实数映射到整数，再在需要时近似还原。 |
| 定点数是什么？ | 小数点位置固定的二进制数，本质上是整数乘一个固定比例。 |
| 为什么 NPU 喜欢量化？ | 低位宽乘法器更小、更省电，数据搬运更少，片上 buffer 能放更多数据。 |
| 为什么不能随便量化？ | 量化会带来舍入、截断、饱和和累加溢出风险，需要校准和位宽设计。 |

量化不是“把模型变小”这么简单。对硬件来说，它是在问：每个数用多少 bit，乘法结果用多少 bit，部分和用多少 bit，最后如何缩放回下一层需要的范围。
...
9. Q3.4 的最小刻度是多少？
10. 在完整例题中，`x_scale=0.1`、`w_scale=0.05`、`acc_int32=58`，真实累加值是多少？
11. 量化如何缓解带宽瓶颈？
...
- 外部来源 TensorFlow Lite quantization specification：`https://www.tensorflow.org/lite/performance/quantization_spec`，支撑 int8 量化、scale、zero point、per-axis/per-tensor 等部署约定。

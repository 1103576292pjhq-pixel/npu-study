...
| 问题 | 本章回答 |
| --- | --- |
| 数学公式怎么落到执行？ | 把 `out[oh][ow] = sum input[oh+r][ow+s] * kernel[r][s]` 拆成 `oh/ow/r/s` 四层循环。 |
| 哪些循环产生输出？ | `oh/ow` 定位输出元素，循环结束后写回 `out[oh][ow]`。 |
| 哪些循环负责累加？ | `r/s` 枚举卷积核内部位置，把多个乘积累到同一个 `sum`。 |
| 和 NPU 有什么关系？ | 循环变量会变成地址发生器、buffer 读写节奏、MAC 触发顺序和部分和生命周期。 |

最短学习路径：先认清 `oh/ow` 与 `r/s` 的分工，再手算一个输出值，最后把循环变量映射到输入地址、权重地址、输出地址和 MAC 阵列。

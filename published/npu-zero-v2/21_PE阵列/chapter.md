...
第 20 章把一个 PE 看清了：它是能保存部分和、接收输入和权重、按节拍执行 MAC 的局部计算格子。本章把许多个 PE 放到一起，讨论“阵列”这件事。PE 阵列不是简单地把 PE 数量相加，而是要回答：这些 PE 排成几行几列？谁负责哪个输出？输入和权重如何送进去？部分和在哪里保存或合并？当 tile 不能整除阵列规模时，哪些 PE 会空闲？

## 本章知识全景图

| 核心问题 | 本章给出的回答 |
| --- | --- |
| PE 阵列是什么？ | 多个 PE 按规则拓扑组织成的并行计算结构，常见是二维网格，也可以是向量 lane、多个子阵列或层次化 cluster。 |
| 阵列如何对应循环？ | 把 `oh/ow/oc` 等输出枚举维度映射到 PE 的行列，把 `ic/r/s/k` 等归约维度多半留在时间上或分批归约。 |
| 阵列组织难在哪里？ | 难点不只是 PE 数量，而是数据供给、部分和归属、负载均衡、边界 tile 和控制同步。 |
| 和脉动阵列是什么关系？ | 脉动阵列是 PE 阵列的一类特定组织方式，本章先讲一般 PE 阵列，下一章再讲脉动数据如何按波前流动。 |

读本章时请持续保留一个问题：一个 PE 已经能算一个输出的一部分，为什么还要把很多 PE 组织成阵列？答案不是“为了看起来更强”，而是为了把卷积、GEMM 中大量相似的输出并行展开，同时让输入、权重和部分和尽可能被复用。

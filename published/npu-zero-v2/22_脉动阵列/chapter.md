...
第 21 章讲了 PE 阵列：很多 PE 可以排成二维网格，共同计算一个输出 tile。脉动阵列是 PE 阵列中非常重要的一类，它的特别之处不是“PE 很多”，而是“数据按固定节拍在相邻 PE 之间流动”。输入、权重或部分和从阵列边界进入，像脉搏一样一拍一拍推进；每个 PE 在本地拿到一对数据后执行 MAC，再把数据或部分和传给下游。

## 本章知识全景图

| 核心问题 | 本章给出的回答 |
| --- | --- |
| 什么是脉动阵列？ | 一种规则 PE 阵列，数据按固定节拍在相邻 PE 间传递，PE 在本地同步执行计算。 |
| 为什么要“脉动”？ | 让每个数据在阵列内多次复用，减少全局广播和外部内存访问，同时保持局部互连。 |
| 难点是什么？ | 数据必须按时间错位注入，才能让正确的 A 和 B 在正确 PE 同一周期相遇。 |
| 与普通 PE 阵列的区别？ | 普通 PE 阵列强调空间并行；脉动阵列进一步规定数据在阵列中的节拍、方向和相邻传递方式。 |

本章会用 2x2 矩阵乘法手算一个最小脉动过程。重点不是记住某个厂商的具体矩阵单元尺寸，而是看懂“同一批数据如何在阵列内被多个 PE 顺路使用”。只要这个节拍直觉建立起来，后续看 TPU、Edge TPU 或 NPU 文档里的 systolic array 就不会只停留在名词层面。
...
3. 为什么脉动阵列强调相邻通信？
4. 什么是时间错位注入？
5. 在 2x2 完整例题中，`C00` 的结果是多少？
6. 在 2x2 完整例题中，`PE(1,1)` 为什么最晚完成？

...
| 问题 | 本章回答 |
| --- | --- |
| 四重循环为什么不够？ | 真实卷积还有输出通道 `oc`、输入通道 `ic`，批处理时还有样本维度 `n`。 |
| 六重循环的核心分工是什么？ | `oc/oh/ow` 产生输出元素，`ic/r/s` 把多个乘积归约成这个输出。 |
| 七重循环多了什么？ | `n` 表示独立样本循环，不参与跨样本累加。 |
| 和硬件有什么关系？ | 循环分工决定权重复用、输入复用、部分和驻留、buffer 分配和 PE 并行维度。 |

最短学习路径：先把 `oc/oh/ow` 与 `ic/r/s` 分成输出循环和归约循环，再加入 batch，最后用这七个坐标判断一个卷积优化到底改变了哪一层。

...
- 本章主题：把多卷积核解释成多输出通道和并行机会。
- 核心概念：C_out / kernel group / output channel / weight tile / output tile
- 逻辑主线：多个卷积核不是重复同一结果，而是每个输出通道拥有一组独立权重，分别对同一输入张量产生不同响应。
- 最小学习路径：一个输出通道 -> 多个 kernel group -> 多输出通道 -> OC tile -> 权重和输出压力。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 输出结构 | output channel | 06 多通道卷积 | 输出 activation buffer |
| 权重结构 | `C_out*C_in*R*S` | 权重体 | 权重 tile 和广播 |
| 并行机会 | 不同输出通道可并行 | MAC/PE | PE 阵列分配 |
| 工程代价 | 输出通道、权重、写回 | buffer | 片上容量和带宽 |

![多个卷积核生成多个输出通道](assets/kernels_to_output_channels.svg)

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“多卷积核_输出通道”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“输出通道由不同 kernel group 生成”和“多输出通道带来并行机会，也带来权重压力”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“从一个 kernel group 手算到多个输出通道”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. 输出通道由不同 kernel group 生成

输入通道决定单个输出值怎么累加，输出通道决定要生成多少组不同权重的响应表。
...
## 2. 多输出通道带来并行机会，也带来权重压力

C_out 增大能提供更多独立输出可并行计算，但权重 tile 和输出写回也同步放大。
...
先算输出通道 0。这个核组和第 06 章可复算链相同，所以结果是：
...
### 术语边界与失败排查：kernel group、output channel、weight tile
...
- 工程后果：漏算 `C_in` 或 `C_out` 会让权重 buffer 容量和 DMA 传输量估计偏小，实际运行时可能频繁分块或溢出。
- 判断线索：看到权重文件大小、shape 或 layer 参数量时，若不能还原到 `C_out*C_in*R*S`，就要先核对通道维度。
...
- 工程后果：一一对应误解会把标准卷积错写成分组卷积或 depthwise 卷积，输出数值和模型语义都会改变。
- 判断线索：如果每个输出通道只读取一个输入通道，除非模型明确是 depthwise/group conv，否则应视为实现口径错误。
...
- 工程后果：如果把输出通道当复制结果，就会错失输入窗口跨 `C_out` 复用的机会，也会错误压缩权重或输出。
- 判断线索：同一输入窗口对应多个核组时，输入可复用但权重和部分和应彼此独立；若多个输出通道数值完全重复，要检查权重索引。
...
- 工程后果：低估 `C_out` 成本会让阵列并行度、权重带宽和输出写回量全被低估，部署时常见表现是权重搬运成为瓶颈。
- 判断线索：当 `C_out` 增大后，若权重读取量和输出写回量没有近似线性增加，说明统计口径可能漏掉了输出通道维度。
...
- 工程后果：盲目加宽通道会让后续层 `C_in` 同步变大，片上 buffer、带宽和延迟压力沿网络传递。
- 判断线索：如果提高 `C_out` 后本层精度收益有限，但下一层 MAC 数和权重数同步上升，就要把通道宽度作为全网络成本而不是单层选择。

## 6. 最后速记

### 本章最该记住的结论

- 一个输出通道对应一组完整权重，而不是对应某一个输入通道。
- `C_out` 放大输出 feature map 数量，也放大权重和写回流量。
- 按输出通道并行的前提是输入数据能被复用、权重能供给、psum 不冲突。
- 输出通道越多不一定越好，模型表达能力、带宽和片上容量必须一起判断。

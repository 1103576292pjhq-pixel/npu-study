...
- 本章主题：把算子支持边界写成图切分、fallback 和编译器约束问题。
- 核心概念：operator support / subgraph / fallback / fusion / layout conversion / compiler boundary
- 逻辑主线：NPU 的支持边界决定模型能否形成连续子图；一旦掉出边界，就会引入 fallback、额外搬运和同步成本。
- 最小学习路径：算子集合 -> 支持/不支持分界 -> 子图切分 -> fallback 代价 -> 编译器和 runtime 处理。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 支持对象 | op list、pattern | 编译器后端 | 能否留在 NPU |
| 切分对象 | subgraph | 图优化 | NPU/CPU 交界 |
| 代价来源 | fallback、layout conversion | runtime | 额外搬运和同步 |
| 优化目标 | fusion、rewrite | 算子边界 | 减少断图 |

![算子支持边界示意图](assets/operator_support_boundary.svg)

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“算子支持边界”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“算子支持边界决定模型能否形成连续 NPU 子图”和“fallback 的代价来自断图、搬运、同步和格式转换”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“用小模型切分支持和不支持子图”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. 算子支持边界决定模型能否形成连续 NPU 子图

不是有 NPU 就能整图跑，真正卡住的是某个 op 是否被后端接受，以及断点会不会把整段流水打散。
...
## 2. fallback 的代价来自断图、搬运、同步和格式转换

每一次从 NPU 掉回 CPU，往往不是只慢一点，而是让一串连续子图多出额外写回、重读和等待。
...
## 4. 编译器/runtime 如何处理 unsupported ops 和 fusion
...
- 工程后果：只看 op 名会把不支持的 dilation、group、layout 或 dtype 误送 NPU，编译阶段失败或运行时 fallback。
- 判断线索：检查 supported ops 时至少列 `op_type + shape + dtype + layout + attributes`；缺属性的支持声明不能直接用于部署判断。
...
- 工程后果：单个不支持节点可能导致 NPU 子图被拆成多段，中间大特征图在 NPU/CPU 之间来回搬，整体延迟暴涨。
- 判断线索：如果编译日志出现 graph partition、fallback、copy in/out 或 reorder，就要评估它影响的是单节点还是整段子图。
...
- 工程后果：中间 fallback 会让 NPU pipeline 中断，触发缓存同步和 layout 转换，常见表现是 NPU 利用率下降但 CPU 占用上升。
- 判断线索：看 fallback 节点的张量大小和位置；大 activation 上的 fallback 比最后小 logits 上的 fallback 危险得多。
...
- 工程后果：依赖自动改写可能引入不透明子图、额外 requantize 或临时 buffer，最终精度或性能不可控。
- 判断线索：如果编译器报告 rewrite、decompose、legalize 或 fallback，必须查看改写后的图，而不是只看原模型。
...
- 工程后果：支持列表很长但主干 Conv/GEMM/MatMul 低效，或者融合差、fallback 多，真实模型仍然慢。
- 判断线索：用目标模型统计高频算子和子图覆盖率；支持列表要和模型分布、融合率、fallback 字节数一起看。

## 6. 最后速记

### 本章最该记住的结论

- 支持边界不是静态名单，而是和 shape、layout、dtype、batch 一起变化的后端能力边界。
- 子图越连续，搬运和同步成本越低；断图越多，性能越接近 host 调度。
- 看支持边界时要同时看 op、pattern 和融合后的图形态。
- 调试模型慢，不只是看单个算子耗时，还要看 fallback 断点在哪里。

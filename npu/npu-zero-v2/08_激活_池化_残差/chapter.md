...
- 本章主题：把激活、池化、残差放到主干卷积之外的算子边界中理解。
- 核心概念：activation / ReLU / pooling / residual / elementwise / shape alignment
- 逻辑主线：这些算子常被当作卷积后的“小步骤”，但它们会改变数值范围、空间尺寸、旁路依赖和融合机会。
- 最小学习路径：ReLU 截断 -> pooling 下采样 -> residual 对齐相加 -> 融合与回退边界。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 数值变换 | ReLU、激活函数 | feature map | epilogue/fusion |
| 空间变换 | pooling、下采样 | 窗口与 stride | 输出 tile 尺寸 |
| 图结构 | residual、旁路相加 | shape 对齐 | 跨分支 buffer |
| 部署判断 | 融合、量化、算子支持 | 算子边界 | 编译器和 runtime |

![激活、池化、残差在一层网络中的位置](assets/activation_pooling_residual_pipeline.svg)

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“激活_池化_残差”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“激活函数改变的是数值范围和后续可融合性”和“pooling 和 residual 分别改变空间规模与图依赖”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“用小张量复算激活、池化和残差”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. 激活函数改变的是数值范围和后续可融合性

ReLU 不是装饰层，它会截断负值并常常成为卷积硬件 epilogue 的一部分。
...
## 2. pooling 和 residual 分别改变空间规模与图依赖

pooling 关心窗口下采样，residual 关心两路张量能否按 shape、layout 和量化尺度对齐。
...
## 4. 这些轻量算子如何影响融合、buffer 和 fallback
...
- 工程后果：漏掉激活会让输出分布和参考模型不一致，量化范围、稀疏率和后续层输入都可能偏离。
- 判断线索：如果卷积数值正确但 ReLU 后负值仍存在，或激活后范围没有变化，就要检查激活是否被漏执行或融合顺序错误。
...
- 工程后果：池化规则错会改变输出值和输出尺寸，后续层可能 shape 不匹配，或者分类结果因局部响应被错误保留而漂移。
- 判断线索：若 max pooling 输出不是窗口最大值，average pooling 没按有效元素平均，或边界窗口处理和框架不同，就先查池化规则。
...
- 工程后果：把 Add 当 concat 会改变通道数和后续权重 shape；把 concat 当 Add 则会丢失一部分特征或触发 shape 错误。
- 判断线索：看到残差节点时先查算子类型、两个输入 shape 和输出 shape；若输出通道数翻倍通常是 concat，不是逐元素 Add。
...
- 工程后果：忽略小算子会让中间张量频繁写回外存，融合机会丢失，最终表现为卷积很快但整层 block 延迟仍高。
- 判断线索：如果 profile 中卷积内核很短，但 ReLU/Pool/Add 前后有大量 memcpy、reorder 或 DMA 往返，就要把小算子按数据生命周期重新审查。
...
- 工程后果：随意插入或替换 ReLU 会改变模型函数，量化时还可能改变激活分布，导致校准范围和精度下降。
- 判断线索：如果模型图里没有 ReLU 而 NPU 融合段自动加了截断，或 ReLU6/LeakyReLU 被当作普通 ReLU，要立即核对算子语义。
...
- 工程后果：旁路张量保存策略不当会占满片上 buffer，或被迫外存写回再读回，带宽和延迟都会上升。
- 判断线索：看到 Add 的第二输入来自较早层时，检查它是否仍在片上、shape/layout/scale 是否匹配，以及是否需要额外 DMA 读回。

## 6. 最后速记

### 本章最该记住的结论

- 激活、池化、残差都不是单纯附属步骤，它们会改变数值、shape 或图依赖。
- ReLU 可融合时能减少一次中间写回；不能融合时可能多一次读写。
- 残差相加必须先解决 shape、layout、dtype 和 scale 对齐。
- 算子越轻量，越要关注搬运和同步开销是否超过计算本身。

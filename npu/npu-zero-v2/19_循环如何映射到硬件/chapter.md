...
- 本章主题：把卷积循环维度分配到时间执行、空间并行和片上数据停留位置。
- 核心概念：loop mapping / spatial mapping / temporal mapping / unroll / tile / dataflow / dependence
- 逻辑主线：硬件映射不是把循环原样搬到 PE 阵列，而是选择哪些维度并行、哪些维度顺序执行、哪些数据留在片上。
- 最小学习路径：循环维度 -> 时间映射/空间映射 -> 依赖和复用约束 -> PE 阵列执行流水。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 循环对象 | OH/OW/OC/IC/R/S | 卷积循环 | 任务拆分维度 |
| 时间映射 | temporal loop | 顺序执行 | 单 PE 或单 tile 内循环 |
| 空间映射 | spatial unroll | 并行 | 多个 PE 同时计算 |
| 约束条件 | 依赖、复用、容量、带宽 | dataflow/tile | 可映射性和利用率 |

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“循环如何映射到硬件”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“mapping 是把循环维度分配给时间和空间”和“并行展开受依赖、复用和片上容量约束”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“比较一个 PE 顺序计算和四个 PE 空间并行”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. mapping 是把循环维度分配给时间和空间

同一段卷积循环可以顺序跑，也可以把部分维度铺到 PE 阵列上；结果相同，数据停留和带宽压力不同。
...
不是所有循环都能无代价并行，归约维度会牵动 psum，空间维度会牵动输入复用和边界空泡。
...
## 4. mapping 如何决定 PE 利用率、复用和 psum 路径
...
- 工程后果：把伪代码顺序等同硬件顺序，会误读 trace 和性能瓶颈；硬件可能同时展开多个输出、分块归约或流水搬运，日志不一定按源码嵌套顺序出现。
- 判断线索：如果硬件 trace 中多个 `oh/ow/oc` 同时推进，或 `ic` 被分成多段累加，就说明 mapping 已经改变了时间和空间组织。
...
- 工程后果：盲目并行归约会产生大量 partial sum 合并流量，规约树、写回端口或 accumulator 可能成为瓶颈。
- 判断线索：并行前先标记维度类型；`oc/oh/ow/n` 展开后通常生成独立输出，`ic/r/s/K` 展开后必须有明确 psum merge 路径。
...
- 工程后果：tile 和 mapping 脱节会让 PE 负责的数据块不匹配 buffer 内容，可能出现输入已搬到片上但阵列消费顺序不对，或部分和跨 tile 合并困难。
- 判断线索：检查每个 tile 内 `M/N/K` 或 `oc/oh/ow/ic` 怎样落到 PE；如果 tile 形状不能解释 PE 并行维度，就说明映射不完整。
...
- 工程后果：分开看会让 dataflow 名称变成口号，无法解释为什么某个循环顺序支持权重驻留、另一个顺序支持输出驻留。
- 判断线索：描述 dataflow 时必须指出它保护的是哪一层循环上的复用；例如 weight-stationary 通常要说明权重跨哪些 `oh/ow/n` 迭代复用。
...
- 工程后果：过度展开会让瞬时带宽需求超过片上和外部供给，PE 大面积空等，能耗随阵列规模上升但吞吐不增。
- 判断线索：增加 PE 后如果 utilization 下降、DMA wait 增加、输出写回端口排队，就说明并行度超过了数据供给能力。

### 误区 6：mapping 只需要画一张阵列示意图

- 错误说法：把循环箭头画到 PE 阵列上，就算完成硬件映射。
- 为什么错：真正的 mapping 还要定义地址生成、buffer 分配、部分和归并、边界 tile、同步和写回顺序。
- 正确理解：阵列图只说明空间分配，完整 mapping 必须同时说明时间循环、存储层级和控制信号。
- 工程后果：只画阵列图会漏掉最容易出错的控制面，实际 RTL 或 runtime 中可能出现尾块漏算、DMA 覆盖未完成数据、psum 合并顺序错误。
- 判断线索：审查 mapping 时按六项核对：PE 负责哪些迭代，DMA 搬哪些 tile，buffer 保存哪些数据，psum 怎样合并，边界块怎样处理，最终输出何时写回。

## 6. 最后速记

### 本章最该记住的结论

- 硬件 mapping 的核心是时间执行、空间并行和数据驻留三件事一起选。
- 输出维度通常更容易空间并行，归约维度并行需要处理 partial sum 合并。
- tile 不是 mapping 之后的附属品，它决定一次映射能放下多少输入、权重和 psum。
- 评价 mapping 要看利用率、搬运量、psum 生命周期和边界空泡，而不是只看阵列图是否好看。

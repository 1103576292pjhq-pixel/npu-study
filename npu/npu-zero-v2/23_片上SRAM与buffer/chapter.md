...
- 本章主题：把片上 SRAM 和 buffer 作为 tile、复用、partial sum 与 DMA 之间的容量和带宽契约。
- 核心概念：on-chip SRAM / input buffer / weight buffer / psum buffer / bank / port / double buffering
- 逻辑主线：片上 SRAM 不是普通缓存标签，而是 NPU 显式管理工作集、承接数据复用和隔离外部带宽压力的关键资源。
- 最小学习路径：存储层级 -> buffer 分工 -> tile working set -> bank/port/带宽约束 -> DMA/dataflow 配合。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 存储层级 | register、SRAM、DRAM | memory hierarchy | PE 附近的数据落点 |
| 逻辑分区 | input/weight/psum/output buffer | 数据角色 | 不同生命周期的片上保留 |
| 容量估算 | working set | tile | 是否放得下当前任务 |
| 带宽结构 | bank、port、double buffer | DMA/dataflow | 能否持续喂饱阵列 |

![片上 SRAM 位于 DRAM 和 PE 阵列之间](assets/sram_buffer_hierarchy.svg)

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“片上SRAM与buffer”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“片上 SRAM 是 PE 与 DRAM 之间的可控工作台”和“buffer 设计同时受容量、端口、bank 和数据生命周期约束”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“用卷积 tile 估算 input、weight 和 psum buffer”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. 片上 SRAM 是 PE 与 DRAM 之间的可控工作台

NPU 把当前 tile 的输入、权重和 psum 放在片上，不是为了名字好听，而是为了少去远处 DRAM 取数。
...
## 2. buffer 设计同时受容量、端口、bank 和数据生命周期约束

容量够只说明能放下，端口和 bank 冲突会决定同一周期能不能把这些数据送到 PE。
...
## 3. 用卷积 tile 估算 input、weight 和 psum buffer
...
## 4. SRAM/buffer 如何支撑 DMA、dataflow 和 psum 保留
...
- 工程后果：无节制增大 SRAM 会挤占 PE 或其他单元面积，还可能让访问时序变慢，最终面积功耗上升但吞吐未必改善。
- 判断线索：评估 SRAM 时同时看容量命中率、访问能耗、端口需求和时序；只有容量指标不能说明它值得增加。
...
- 工程后果：按 cache 心智使用 scratchpad 会漏掉显式 load/store 和 ownership，可能让 PE 读到未搬入或已覆盖的数据。
- 判断线索：如果软件需要配置 DMA 描述符、tile base、buffer id 或 ready/done 状态，就说明这是显式 buffer，不是透明 cache。
...
- 工程后果：容量充足但端口不足时，PE 仍会因为读不到输入/权重或写不出 psum 而停顿。
- 判断线索：SRAM occupancy 不高但 stall 高，或 bank conflict 计数上升时，问题不在容量，而在组织和访问并发。
...
- 工程后果：混放没有保护会让 DMA 覆盖未完成 psum，或让权重和输入争用同一端口，造成错误和性能下降。
- 判断线索：查看 buffer map 时必须能指出 input/weight/psum 的地址范围、读写时机和所有者；说不清就有覆盖风险。
...
- 工程后果：buffer 生命周期错会造成断图、边界块错、通道混写或输出写回错位，比单纯慢更危险。
- 判断线索：输出只在 tile 边界、通道切换或 ping-pong 切换时错误，优先查 buffer ownership 和地址映射。

## 6. 最后速记

### 本章最该记住的结论

- 片上 SRAM 的价值来自近、快、省能和可控，但容量和端口都有限。
- input、weight、psum/output buffer 服务的数据生命周期不同，不能只用一个“大缓存”粗略理解。
- working set 估算必须同时包含输入、权重、psum、输出和双缓冲预留。
- buffer 是否有效要看容量、bank/端口、DMA 重叠和 dataflow 是否一起成立。

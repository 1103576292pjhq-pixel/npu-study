...
- 本章主题：把面积、功耗和性能写成同一架构决策的三张账。
- 核心概念：area / power / performance / TOPS / utilization / energy / frequency
- 逻辑主线：PPA 不是三个彼此独立的指标，而是同一设计选择在三张账上的不同后果；优化一个方向，通常会牵动另外两个方向。
- 最小学习路径：目标场景 -> 面积/功耗/性能约束 -> 资源和频率选择 -> 延迟与能量核算 -> 设计决策。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 面积对象 | MAC、SRAM、互连 | 芯片资源 | 能放多少硬件 |
| 功耗对象 | 动态/静态功耗 | 电压频率 | 能否长期运行 |
| 性能对象 | latency、throughput、utilization | 执行时间 | 能否满足场景 |
| 决策对象 | PPA tradeoff | 架构选择 | 目标设备与模型 |

![PPA 三角取舍](assets/ppa_tradeoff_triangle.svg)

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“面积功耗性能取舍”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“PPA 是同一架构决策的三张账，不是三个独立指标”和“峰值性能必须和带宽、利用率、能耗一起解释”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“用两个 NPU 方案比较延迟和能量”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. PPA 是同一架构决策的三张账，不是三个独立指标

加 MAC、加 SRAM 或提频率，都会同时改写面积、功耗和性能，只是三张账的方向不同。
...
## 2. 峰值性能必须和带宽、利用率、能耗一起解释

只看 TOPS 会漏掉 dataflow、fallback、SRAM 端口和实际利用率，最后得到的可能只是纸面快、系统慢。
...
下面每个误区都按同一条判断链处理：先指出错误说法，再解释它会破坏哪条工程约束，最后给出能在方案评审或 profiling 里使用的判断线索。
...
- 工程后果：盲目加 PE 会让面积和功耗先上升，真实延迟却因为 DMA 饥饿、SRAM 端口不足或 tile 太小而不降反升。
- 判断线索：看 profiling 时不要只看 peak TOPS，先看 PE idle 比例、DMA wait 时间、bank conflict 和有效 MAC/cycle；如果阵列经常等数据，瓶颈不在 MAC 数量。
...
- 工程后果：SRAM 过小会频繁访问外存，SRAM 过大会挤占 PE、互连和控制面积，还可能增加漏电与访问延迟。
- 判断线索：先算目标层的 input/weight/psum/output tile 是否能放下，再看外存访问是否随 SRAM 增大明显下降；如果复用率不提高，继续加 SRAM 只是加成本。
...
- 工程后果：用 TOPS 选型容易选到“峰值好看但模型不合适”的 NPU，尤其是模型包含大量 fallback、动态 shape、layout 转换或小 batch 时。
- 判断线索：同一模型同一输入下比较端到端 latency、energy per inference、NPU 子图连续性和 DRAM traffic；如果 TOPS 高但端到端慢，说明峰值没有被工作负载吃到。
...
- 工程后果：只降频可能让单次推理时间变长，能量反而不降；对实时任务还可能触发排队、降帧或 thermal throttling 后的性能抖动。
- 判断线索：比较的是 energy per inference，不只是瞬时功耗；如果功耗降低 20%，执行时间增加 50%，总能量和用户体验可能更差。
...
- 工程后果：后期再压功耗或面积，常常只能砍频率、砍 SRAM、关功能或限制模型范围，结果破坏原本的性能目标。
- 判断线索：方案评审时把每个架构改动同时写出 area cost、power cost、performance gain 和验证风险；如果一个改动只能说明收益，不能说明代价，它还不是 PPA 决策。

## 6. 最后速记

### 本章最该记住的结论

- PPA 不是后处理，而是架构早期就必须同时考虑的约束。
- 性能必须落到延迟、吞吐、利用率和能量/推理上看。
- 面积、功耗和性能之间没有免费午餐，资源增大通常会带来代价。
- 设计评审时要写清目标设备、工作负载、约束和每项取舍的后果。

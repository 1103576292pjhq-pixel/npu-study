...
- 本章主题：把 stride、padding 和输出尺寸连接到窗口覆盖、地址生成和边界处理。
- 核心概念：stride / padding / output size / SAME / VALID / dilation-free conv / address generation
- 逻辑主线：stride 决定窗口移动步长，padding 决定边界外是否补值，输出尺寸公式只是窗口覆盖规则的压缩写法。
- 最小学习路径：合法窗口 -> stride 移动 -> padding 边界 -> 输出尺寸公式 -> 地址生成检查。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 窗口规则 | kernel、stride、padding | 单通道卷积 | 地址发生器的访问模式 |
| 尺寸推导 | OH、OW 公式 | shape | 输出 buffer 分配 |
| 边界处理 | SAME、VALID、补零 | 卷积边界 | 边界 tile 和 padding 逻辑 |
| 工程检查 | 框架口径、地址生成 | 索引 | 部署一致性 |

![stride、padding 与输出尺寸关系](assets/stride_padding_output_shape.svg)

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“stride_padding_输出尺寸”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“stride 和 padding 改的是窗口覆盖规则”和“输出尺寸公式必须回到窗口是否完整覆盖”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“从窗口移动手算输出高宽”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. stride 和 padding 改的是窗口覆盖规则

输出尺寸不是靠感觉估计，而是由输入边界、补边、kernel 大小和窗口步长共同决定。
...
## 2. 输出尺寸公式必须回到窗口是否完整覆盖

公式里的每一项都对应一个边界动作，漏掉 padding 或 stride 会让输出 shape 和地址全部错位。
...
- 工程后果：padding 只按“凑尺寸”处理会导致边界值和参考框架不一致，严重时还会让后续残差相加的 shape 对不上。
- 判断线索：如果输出中心区域正确、边缘一圈错误，或 `SAME/VALID` 切换后尺寸符合但数值不符，优先检查 padding 的 begin/end 和补值规则。
...
- 工程后果：stride 误设会让输出分辨率下降、检测小目标能力变差，也会改变后续层输入 shape，造成模型导入或残差路径失败。
- 判断线索：如果输出 `OH/OW` 比预期小、特征图空间细节明显减少，或后续层 shape mismatch，先核对 stride 是否被错误放大。
...
- 工程后果：输出尺寸估错会让 buffer 分配、DMA 写回长度和后续算子输入全部错位，可能表现为越界、截断或多写无效元素。
- 判断线索：看到公式中的 `(I+2P-K)/S+1` 不能整除，或框架使用 `ceil`/`floor` 规则不同，就必须逐项核对合法窗口起点。
...
- 工程后果：把 padding 0 当真实像素会误读边界响应，调试量化或归一化时也可能把人为补值混进数据分布判断。
- 判断线索：如果边界窗口读到的 0 没有对应原始坐标，或地址生成器在越界时注入常数而非发起真实读，就应把它标为 padding 值。
...
- 工程后果：强行物理扩图会增加外存读写和临时 buffer，尤其在大分辨率输入上会浪费带宽和片上容量。
- 判断线索：如果实现日志出现完整 padded tensor 写回外存，或 padding 后输入字节数明显增加，就要检查是否能改为边界注入或片上填零。

## 6. 最后速记

### 本章最该记住的结论

- stride 控制窗口移动步长，padding 控制边界外补值口径。
- 输出尺寸公式是窗口覆盖规则的压缩表达，不能脱离具体框架口径。
- padding 不一定要真的复制一圈大输入，硬件可在地址生成或边界逻辑中处理。
- 输出尺寸错会传导到 buffer 大小、DMA 地址、下一层 shape 和编译器任务描述。

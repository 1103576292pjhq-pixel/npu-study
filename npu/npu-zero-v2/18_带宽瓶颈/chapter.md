...
- 本章主题：把带宽瓶颈从一句性能口号改写成 MAC 需求和数据供给的字节账。
- 核心概念：bandwidth / arithmetic intensity / roofline / DRAM traffic / SRAM bandwidth / utilization
- 逻辑主线：NPU 跑不满常常不是 MAC 不够，而是输入、权重、部分和和输出不能按阵列消耗速度送达；判断瓶颈要同时算 MAC 和 bytes。
- 最小学习路径：峰值 MAC -> 每周期字节需求 -> 无复用/有复用对比 -> tile/buffer/DMA/dataflow 缓解。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 性能对象 | TOPS、MAC utilization | MAC/PE | 阵列理论吞吐 |
| 搬运对象 | input、weight、psum、output bytes | 数据复用 | DRAM/SRAM/DMA 流量 |
| 判断口径 | 计算强度、roofline | 除法和单位换算 | 算力瓶颈或带宽瓶颈定位 |
| 缓解手段 | tile、buffer、dataflow、量化 | 片上存储 | 减少外部搬运和阵列空等 |

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“带宽瓶颈”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“带宽瓶颈发生在数据供给慢于 MAC 消耗”和“计算强度把 MAC 数和搬运字节放到同一张账上”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“用无复用和有复用对比卷积 tile 的字节账”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. 带宽瓶颈发生在数据供给慢于 MAC 消耗

当 PE 每周期要吃的输入、权重和 psum 字节数超过存储系统能送来的字节数，峰值 TOPS 就会变成空等。
...
## 2. 计算强度把 MAC 数和搬运字节放到同一张账上

同一个算子是否带宽受限，不看名字，而看每搬 1 byte 能支撑多少次 MAC。
...
## 4. tile、buffer、DMA 和 dataflow 如何缓解带宽瓶颈
...
- 工程后果：只看 TOPS 会误判性能上限，实际可能出现 PE 利用率低、DMA 等待高、增加 PE 后延迟几乎不下降。
- 判断线索：profile 中若外部带宽接近上限而 MAC 利用率不高，说明瓶颈是供数，不是峰值算力。
...
- 工程后果：只扩 DRAM 带宽可能无效，因为片上 buffer 端口、bank 冲突或互连广播能力仍会限制每周期供数。
- 判断线索：外部带宽未满但 PE 仍等数据时，继续查 SRAM 端口、bank conflict、片上网络和寄存器读写端口。
...
- 工程后果：只按容量切 tile 会错过复用，甚至让 DMA 变成大量小块搬运，带宽效率下降。
- 判断线索：如果 tile 都能放下但外存 byte 数仍高、DMA 请求碎片多，就要检查 tile 形状是否支持连续搬运和复用。
...
- 工程后果：只减少 MAC 可能引入更多重排或临时写回，最终延迟、能耗不降反升。
- 判断线索：当优化后 MAC 数下降但 byte 数上升，且总耗时不降，要用计算强度和 roofline 口径重新判断。
...
- 工程后果：优先级错会把片上空间给低收益数据，真正高代价的数据反而反复外存往返。
- 判断线索：逐类统计输入、权重、部分和的外存字节数和复用次数；最高字节数或最宽部分和通常是第一优先检查对象。

## 6. 最后速记

### 本章最该记住的结论

- TOPS 是计算上限，真实吞吐还受 DRAM、SRAM、DMA 和阵列端口供数限制。
- 带宽瓶颈要用 bytes/MAC 或 MAC/byte 估算，不能只凭模型层数猜。
- 输入、权重和 psum 的复用能把外部读写压到片上局部访问。
- 好的调度不是只少搬，还要提前搬、连续搬，并让搬运和计算重叠。

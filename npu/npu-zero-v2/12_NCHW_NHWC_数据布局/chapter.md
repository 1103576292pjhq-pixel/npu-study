...
- 本章主题：把 NCHW/NHWC 从名字差异改写成内存访问和 DMA 连续性的差异。
- 核心概念：NCHW / NHWC / layout / stride / contiguous / transpose / blocked layout
- 逻辑主线：layout 不改变张量数学含义，但会改变相邻元素是谁、DMA 怎样搬、PE 怎样取数和转换代价在哪里。
- 最小学习路径：shape 语义 -> NCHW/NHWC 地址顺序 -> layout 转换 -> DMA/PE 访问判断。

### 2. 概念地图

| 概念层级 | 核心概念 | 前置知识 | 延伸到 NPU 的位置 |
| --- | --- | --- | --- |
| 逻辑形状 | N/C/H/W | 02 shape | 模型图张量边 |
| 存储顺序 | NCHW、NHWC | 数组地址 | DMA stride 和连续读取 |
| 转换代价 | transpose、reorder | 内存搬运 | 编译器 layout 策略 |
| 硬件格式 | blocked layout | tile | 向量化和 PE 对齐 |

### 3. 阅读顺序 / 处理顺序

- 先建立什么直觉：先把“NCHW_NHWC_数据布局”落到一个可观察、可手算或可追踪的对象上。
- 再形式化什么定义或流程：围绕“layout 决定内存相邻方向，不决定数学值”和“layout 转换是一次真实搬运，不是改个标签”整理概念边界、输入输出和约束。
- 最后通过什么例子、操作或推导固化：用“用哨兵小张量复算 NCHW 与 NHWC 地址”进入复现链，再回到 NPU 的 MAC、buffer、DMA、PE、编译器或 runtime 判断。
## 1. layout 决定内存相邻方向，不决定数学值

NCHW 和 NHWC 装的是同一批数，但内存里谁挨着谁不同，NPU 取数方式也会不同。
...
## 2. layout 转换是一次真实搬运，不是改个标签

如果数据已经按一种顺序存好，换成另一种 layout 往往要读出再重排写回。
...
- 工程后果：layout 看错会让 DMA 从连续搬运变成跨步访问，或者让 PE 一拍需要的通道数据分散在内存远处。
- 判断线索：如果逻辑 shape 相同但 burst 长度、cache line 命中和向量加载效率差异明显，先检查 NCHW/NHWC 的线性 offset。
...
- 工程后果：把正确 layout 转换当成数值变换，会误判部署误差；真正危险的是转换后索引解释不一致，导致通道和空间位置错配。
- 判断线索：转换前后按同一逻辑坐标读取，应得到同一元素；若元素总数相同但坐标对应表对不上，就是转换或解释错了。
...
- 工程后果：盲目改 NHWC 可能插入大量转换，或让本来沿宽度连续的访问变差，最终收益被重排成本吞掉。
- 判断线索：只有当硬件内层消费通道、向量宽度匹配 `C` 块、且转换成本能被后续算子摊薄时，NHWC 才可能是更好选择。
...
- 工程后果：用 reshape 冒充 transpose 会把通道数据读成空间数据，模型输出可能大幅漂移，但张量元素数量和内存大小看起来都正常。
- 判断线索：如果转换操作没有按 permute 规则移动数据，却声称完成 NCHW 到 NHWC，就要用一个 1x3x2x2 小张量逐元素核对。
...
- 工程后果：layout 转换过多会形成“算子快、整网慢”的现象，尤其在小算子之间来回转换时，带宽和临时 buffer 成本会超过计算本身。
- 判断线索：profile 里如果 reorder/transpose/memcpy 时间接近或超过卷积/GEMM 时间，就要检查全图 layout 规划，而不是只优化单个算子。

## 6. 最后速记

### 本章最该记住的结论

- shape 描述每维含义，layout 描述这些维度在内存里的排列顺序。
- NCHW/NHWC 的数学元素相同，但连续访问方向不同。
- layout 转换可能成为整张量读写，不能当成免费操作。
- 检查 layout 错误时，优先用哨兵值定位轴顺序和地址顺序。
